...
int do_open(char *pathname, int flags) {
  ...
  if(pathname[0] == '.' && pathname[1] == '.'){
      ...
      if(current->pfiles->cwd->parent != NULL){
        strcpy(absolutePath, current->pfiles->cwd->parent->name);
        if(0 == strcmp("/", absolutePath)){
          absolutePath[0] = 0;
        }
      ...
      strcpy(absolutePath, current->pfiles->cwd->name);
      if(0 == strcmp("/", absolutePath)){
          absolutePath[0] = 0;
        }
      pathname++;
  ...
  if ((opened_file = vfs_open(absolutePath, flags)) == NULL) return -1;
...
//pwd
int do_rcwd(char* pathVA) {
  if (current->pfiles->cwd == NULL){
    return -1;
  }
  char *pathPA = (char *)user_va_to_pa((pagetable_t)current->pagetable, pathVA);
  strcpy(pathPA, current->pfiles->cwd->name);
  return 0;
}

//cd
int do_ccwd(char* pathVA) {
  if (current->pfiles->cwd == NULL){
    return -1;
  }
  ...
  if (pathPA[0] == '.' && pathPA[1] == '.') {
    ...
    if (current->pfiles->cwd->parent != NULL) {
        ...
        current->pfiles->cwd->parent = current->pfiles->cwd->parent->parent;
        free_page(p);
    ...
    if(0 == strcmp("/",current->pfiles->cwd->name )){
